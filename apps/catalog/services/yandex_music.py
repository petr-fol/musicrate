...
Provides search and import functionality for Yandex Music.
"""
...
class YandexMusicProvider:
    ...
    def search(self, query, limit=10):
        ...
        Returns:
            Список словарей с информацией о релизах
        """
        results = []
    ...
    def import_release(self, yandex_id):
        """
        Импорт релиза из Яндекс.Музыки в локальную базу данных.

        Args:
            yandex_id: ID релиза в Яндекс.Музыке

        Returns:
            Объект Release или None
        """
        try:
            ...
            release_type = self._determine_release_type(album)
    ...
    def _determine_release_type(self, album):
        """
        Определить тип релиза на основе метаданных.
    ...
    def _parse_release_date(self, date_value):
        """
        Парсинг даты релиза из различных форматов.

        Args:
            date_value: Значение даты

        Returns:
            date object или None
        """
        if not date_value:
            return None
    ...
    def _download_image(self, instance, url, field_name):
        """
        Загрузка изображения из URL и сохранение в поле модели.
